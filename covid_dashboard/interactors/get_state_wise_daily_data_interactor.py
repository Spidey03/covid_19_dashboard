...
class GetStateWiseDailyReport:
    ...
    def get_state_wise_daily_report(self):

        daily_state_report_dto = self.storage.\
            get_state_wise_daily_report()
        response = self.presenter.\
            get_response_for_state_wise_daily_report(daily_state_report_dto)
        return response